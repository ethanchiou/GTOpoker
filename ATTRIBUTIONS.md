...
|---|---|---|---|
| [postflop-solver](https://github.com/b-inary/postflop-solver) | Heads-up postflop CFR solver. Wired in Phase 2 via the `packages/solver-worker` WASM glue crate; built separately per `solver-worker/BUILD.md`. | **AGPL-3.0** | Network-copyleft; acceptable for this open-source project. The WASM artifact is not committed (rebuild from source). |
